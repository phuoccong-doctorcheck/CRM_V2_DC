...
// // eslint-disable-next-line import/order
// import { clearKPIDays, getKPIDays } from "store/kpi_taskview";
// // eslint-disable-next-line import/order
...
//       mergedObject["employee_id"] = item.data.employee_id;
//       mergedObject["date"] = dayjs(item.data.date).format("DD-MM");
...
//   useEffect(() => {
//     if (listKPIDay1.length !== 0) {
//       const result = processData(listKPIDay1);
...
//   const updatedDataCustom = dataCustom.map((item) => {
//     const isDisabled = kpiEmployeeDetails
//       ? kpiEmployeeDetails.some(
//           (kpi) => kpi.kpi_name.trim() === item.label.trim()
//         )
...
//                       <Typography
//                         content={(
//                           kpiEmployee?.details[0]?.target_customer +
//                           kpiEmployee?.details[1]?.target_customer +
//                           kpiEmployee?.details[2]?.target_customer +
//                           kpiEmployee?.details[3]?.target_customer
...
//                       <Typography
//                         content={(
//                           kpiEmployee?.details[0]?.real_customer +
//                           kpiEmployee?.details[1]?.real_customer +
//                           kpiEmployee?.details[2]?.real_customer +
//                           kpiEmployee?.details[3]?.real_customer
//                         )?.toLocaleString("vi-VN")}
//                         modifiers={["13x18", "600", "center", "cg-red"]}
//                       />
//                     </Table.Summary.Cell>
...
//                       <Typography
//                         content={Math.floor(
//                           kpiEmployee?.details[0]?.target_revenue +
//                             kpiEmployee?.details[1]?.target_revenue +
//                             kpiEmployee?.details[2]?.target_revenue +
//                             kpiEmployee?.details[3]?.target_revenue
//                         ).toLocaleString("vi-VN")}
//                         modifiers={["13x18", "600", "center", "cg-red"]}
//                         styles={{ textAlign: "end", marginRight: "12px" }}
//                       />
...
//                       <Typography
//                         content={Math.floor(
//                           kpiEmployee?.details[0]?.real_revenue +
//                             kpiEmployee?.details[1]?.real_revenue +
//                             kpiEmployee?.details[2]?.real_revenue +
//                             kpiEmployee?.details[3]?.real_revenue
//                         ).toLocaleString("vi-VN")}
//                         modifiers={["13x18", "600", "center", "cg-red"]}
//                         styles={{ textAlign: "end", marginRight: "12px" }}
//                       />
...
//                       <Typography
//                         content={Math.floor(
//                           kpiEmployee?.details[0]?.target_commision +
//                             kpiEmployee?.details[1]?.target_commision +
//                             kpiEmployee?.details[2]?.target_commision +
//                             kpiEmployee?.details[3]?.target_commision
//                         ).toLocaleString("vi-VN")}
//                         modifiers={["13x18", "600", "center", "cg-red"]}
//                         styles={{ textAlign: "end", marginRight: "12px" }}
//                       />
...
// content={
//   (
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.new_inbox_count || 0) +
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[1]?.new_inbox_count || 0)
//   ) == 0
//     ? "-"
//     : (
//         (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.new_inbox_count || 0) +
...
//                          content={
//   (
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.old_inbox_count || 0) +
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[1]?.old_inbox_count || 0)
//   ) == 0
//     ? "-"
//     : (
//         (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.old_inbox_count || 0) +
...
//                              content={
//   (
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.phone_number_count || 0) +
//     (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[1]?.phone_number_count || 0)
//   ) == 0
//     ? "-"
//     : (
//         (kpiEmployeeMonth?.details?.[0]?.statistic_by_page?.[0]?.phone_number_count || 0) +
...
//                            content={
//   kpiEmployeeMonth?.details?.[0]?.customer_appointments_count == null
//     ? "-"
//     : kpiEmployeeMonth.details[0].customer_appointments_count.toLocaleString("vn-VN")
// }
...
//                        <Typography
//                             content={
//   kpiEmployeeMonth?.details?.[0]?.customer_checkin_count == null
//     ? "-"
//     : kpiEmployeeMonth.details[0].customer_checkin_count.toLocaleString("vn-VN")
// }
...
//                          modifiers={["13x18", "600", "center", "cg-red"]}
//                          styles={{ textAlign: "end", marginRight: "0x" }}
//                       />
//                      </Table.Summary.Cell>
//                      <Table.Summary.Cell
//                       index={6}
...
//                       content={
//   kpiEmployeeMonth?.details?.[1]?.customer_contacts_count == null
//     ? "-"
//     : kpiEmployeeMonth.details[1].customer_contacts_count.toLocaleString("vn-VN")
// }
...
//                        <Typography
//                              content={
//   kpiEmployeeMonth?.details?.[1]?.customer_add_service_retail_count == null
//     ? "-"
//     : kpiEmployeeMonth.details[1].customer_add_service_retail_count.toLocaleString("vn-VN")
// }
//                          modifiers={["13x18", "600", "center", "cg-red"]}
...
//                        <Typography
//                           content={
//   kpiEmployeeMonth?.details?.[1]?.customer_checkin_count == null
//     ? "-"
//     : kpiEmployeeMonth.details[1].customer_checkin_count.toLocaleString("vn-VN")
// }
...
//     dispatch(clearKPIDays());
//     if (selectedDates?.length !== 0) {
  
//       for (const item of selectedDates) {
//         // Đợi cho dispatch hoàn tất
//         await dispatch(
//           getKPIDays({
//             employee_id: states.employee_id,
//             employee_code: states.employee_code,
...
//         // Gọi hàm để xóa item sau khi dispatch thành công
//         handlePrintAndRemove();
//       }
//        dispatch(
//                         getKPIEmployeeMonth({
//                           //  ...propsData,
...
//        for (const item of removedDates) {
//         // Đợi cho dispatch hoàn tất
//         await dispatch(
//           getKPIDays({
//             employee_id: states.employee_id,
...
//                      const selectedDate = moment(date); // Lấy giá trị tháng được chọn
...
//       setIsFutureMonth(true); // Nếu là quá khứ, set thành false
//     } else {
//       setIsFutureMonth(false); // Nếu không, set thành true
//     }
//                     setSelectedDates([])
//                     if (date) {
...
//                         // Log các ngày từ 1 đến ngày xác định (daysInMonth)
//                         for (let day = 1; day <= daysInMonth; day++) {
//                           const formattedDate = moment(
//                             `${selectedYear}-${selectedMonth + 1}-${day}`,
//                             "YYYY-MM-DD"
//                           ).format("YYYY-MM-DD");
//                           newDates.push(formattedDate); // Thêm từng ngày vào mảng mới
//                         }

//                         return [...new Set([...prevDates, ...newDates])]; // Gộp mảng cũ và mới, loại bỏ trùng lặp
//                       });
//                     }
//                   }}
//                   variant="simple"
...
//   {kpiEmployee?.details?.length !== 4 && (
//     <>
//       {isFutureMonth === false && modeButtonAdd === true && (
//         <Button
//           onClick={() => {
//             setSendKPI({
//               ...sendKPI,
...
//         <div style={{ marginBottom: "10px" }}>
//           <Dropdown2
//             dropdownOption={updatedDataCustom || []}
