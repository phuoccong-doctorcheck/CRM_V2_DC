...
import { getListTask, getListTaskE, getListTaskFlo } from "store/tasks";
...
import TaskSummaryPopup from './TaskSummaryPopup/TaskSummaryPopup';
...
const statusTask = [
  { id: 1, label: "Tất cả trạng thái" ,value :"all"},
  { id: 2, label: "Đang thực hiện" ,value :"inprogress"},
  { id: 3, label: "Đã hoàn thành" ,value :"done"},
  { id: 4, label: "Dời ngày" ,value :"delay"},
  { id: 5, label: "Đã hủy" ,value :"canceled"}
]
const BeforeMedicalExamination: React.FC = () => {
  ...
  const listTaskFlo = useAppSelector((state) => state.listTaskReducer.taskListFlo);
  const loadingListTaskFlo = useAppSelector((state) => state.listTaskReducer.loadingTaskListFlo);
  ...
  const storeTeamIDS = localStorage.getItem("team_ids");
    const storeRoleUser = localStorage.getItem("role_user");
  ...
  const [listRoles] = useState(getRoles ? JSON.parse(getRoles) : '');
   const [roleUser] = useState(storeRoleUser ? JSON.parse(storeRoleUser) : '');
...
const [stateEmployeeId, setStateEmployeeId] = useState<any>(
  ...
  const [userguidType, setListUserguidType] = useState<any[]>(
    storageGroupTask ? JSON.parse(storageGroupTask) : []
  );
  ...
  const [dataListTaskFlo, setDataListTaskFlo] = useState(listTaskFlo || []);
  ...
  console.log(dataListTaskE?.data)
  ...
  const bodyGetList = {
    source_id: filterData?.source_id || "",
    step_index: filterData?.step_index?.value || "",
    ...
    is_abandoned:filterData.is_abandoned,
    abandoned_days: filterData.abandoned_days,
  };
 ...
 const [filterTaskAll, setFilterTaskAll] = useState({
  ...
  useEffect(() => {
    ...
    useEffect(() => {
    setDataListTaskFlo(listTaskFlo);
  }, [listTaskFlo]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    document.title = "Danh sách khách hàng tiềm năng | CRM";
  }, [listBeforeExams]);
  ...
  const handleChangeStatusTask = () => {
    ...
    console.log(stateChangeStatusTask)
  ...
  console.log(moment(filterData?.from_date).format("YYYY-MM-DD 00:00:00"))
  //
      ...
      onSuccess: (data) => {
        if (data.status) {
          ...
          toast.success(
           ...
           window.location.href = `/lead-info?id=${data.data.lead_id}`
      //    dispatch(
      //      getListToStoreBeforeExams({
      //  ...bodyGetList,
  ...
  const [stateStyleOfTask, setStateStyleOfTask] = useState("")
  ...
  const tableColumns = [
    ...
    {
      title: (<Typography content="Ngày cập nhật" modifiers={["14x20", "500", "center", "main"]} />),
      ...
      {
      title: (<Typography content="Nickname" modifiers={["14x20", "500", "center", "main"]} styles={{textAlign:"left", marginLeft:"12px"}}/>),
      dataIndex: "lead_nickname",
      align: "center",
    ...
    {
      title: (<Typography content="Tags" modifiers={["14x20", "500", "center", "main"]} styles={{
        marginLeft:15
      }}/>),
      dataIndex: "tags",
      align: "center",
      width:200,
      className: "ant-table-column_wrap",
      // filters: stateTagsCustomer.map((group) => {
      //   const obj = { text: group.label, value: group.value }
      //   return obj;
      // }),
      // onFilter: (value: any, record: any) => {
      //   return record?.tag_id === value
      // },
      render: (record: any[], data: any) => (
        <div
        className="ant-table-column_item"
      ...
      >
          <div className="ant-table-column_item__tags" style={{
            display: "flex",
            flexWrap: "wrap",
            gap: 4,
            maxWidth: 200,
          ...
          {record && record.length > 0 ? (
            record.map((tag: any, index: number) => {
              const tagInfo = stateTagsCustomer.find(item => item.tag_id === tag.tag_id);
              return (
                <div
                  key={index}
                  className="ant-table-column_item__tag"
                  style={{
                    backgroundColor: tagInfo?.tag_color || "transparent",
                    borderRadius: 3,
                    padding: "1px 5px",
            ...
            <div className="ant-table-column_item__tag">---</div>
          )}
        </div>
      </div>
  ...
  
    {
      ...
      onFilter: (value: any, record: any) => {
        ...
        return record?.source_group_id === value
      },
      className: "ant-table-column_wrap",
      render: (record: any, data: any) => (
        <div className="ant-table-column_item" onClick={() => {
         window.location.href = `/lead-info?id=${data.lead_id}`;
          ...
          <Typography content={stateLaunchSourceGroups.find(item => item.id === record)?.label} modifiers={["14x20", "400", "center"]} />
        </div>
      ),
    },
      ...
      render: (record: any, data: any) => (
        <div className="ant-table-column_item" onClick={() => {
         window.location.href = `/lead-info?id=${data.lead_id}`;
          ...
          <Typography content={stateLaunchSource.find(item => item.id === record)?.label} modifiers={["14x20", "400", "center"]} />
        </div>
      ),
    },
    {
      title: (<Typography content="Chuyển đổi" modifiers={["14x20", "500", "center", "main"]} />),
    ...
    {
      title: (<Typography content="Giai đoạn" modifiers={["14x20", "500", "center", "main"]} />),
      dataIndex: "step_id",
      filters: stepsprocesslead.map((group) => {
        const obj = { text: group.label, value: group.value }
        return obj;
      }),
      onFilter: (value: any, record: any) => {
        console.log(value,record)
        return record?.step_id === value
      },
      align: "center",
      width: 150,
      className: "ant-table-column_wrap",
      render: (record: any, data: any) => (
        <div className="ant-table-column_item" onClick={() => {
         ...
         <Typography content={stepsprocesslead.find(item => item.id === record)?.label} modifiers={["14x20", "400", "center"]} />
        </div>
      ),
    },
    {
      title: (<Typography content="Nhân viên phụ trách" modifiers={["14x20", "500", "center", "main"]} />),
      dataIndex: "follow_employee_id",
      align: "center",
...
>
  <Typography
    content={
      (() => {
        const matched = listUsers.find(emp => emp.u_id === record);
  ...
  />
     ...
     {
      title: (<Typography content="Tương tác lần cuối" modifiers={["14x20", "500", "center", "main"]} />),
      dataIndex: "lead_last_datetime",
      align: "center",
      width: 120,
      className: "ant-table-column_wrap",
      render: (record: any, data: any) => (
        <div className="ant-table-column_item"
          onClick={() => {
            handlePostInteract(data.lead_id)
        ...
        >
         ...
         content={record ? moment(record).format('hh:mm DD/MM/YYYY') : "---"}
            modifiers={["12x20", "500", "center", "uppercase"]}
          />
            </div>
        </div>
      ),
    ...
    {
      title: (<Typography content="Link Pancake" modifiers={["14x20", "500", "center", "main"]} />),
      dataIndex: "lead_pancake_link",
      align: "center",
      width: 80,
      className: "ant-table-column_wrap",
      render: (record: any, data: any) => (
        <div className="ant-table-column_item"
          onClick={() => {
            if (record === null) {
              ...
              window.open(record, '_blank', 'noopener,noreferrer');
          }
         
        }}
        >
         <div
><svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M15.197 3.35462C16.8703 1.67483 19.4476 1.53865 20.9536 3.05046C22.4596 4.56228 22.3239 7.14956 20.6506 8.82935L18.2268 11.2626M10.0464 14C8.54044 12.4882 8.67609 9.90087 10.3494 8.22108L12.5 6.06212" stroke="#00556e" stroke-width="1.5" stroke-linecap="round"></path> <path d="M13.9536 10C15.4596 11.5118 15.3239 14.0991 13.6506 15.7789L11.2268 18.2121L8.80299 20.6454C7.12969 22.3252 4.55237 22.4613 3.0464 20.9495C1.54043 19.4377 1.67609 16.8504 3.34939 15.1706L5.77323 12.7373" stroke="#00556e" stroke-width="1.5" stroke-linecap="round"></path> </g></svg></div>
        </div>
      ),
    },
      {
      title: (<Typography content="Công việc" modifiers={["14x20", "500", "center", "main"]} />),
      dataIndex: "lead_pancake_link",
      align: "center",
      width: 70,
      className: "ant-table-column_wrap",
      render: (record: any, data: any) => (
        <div className="ant-table-column_item"
          onClick={() => {
            ...
            setFilterTask({
              ...filterTask,id: data.lead_id,
            })
            dispatch(getListTask({ ...filterTask, id: data.lead_id} as unknown as any));
        }}
        >
         <div
          >
            <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path opacity="0.4" d="M12.3691 8.87988H17.6191" stroke="#00556e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M6.38086 8.87988L7.13086 9.62988L9.38086 7.37988" stroke="#00556e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M12.3691 15.8799H17.6191" stroke="#00556e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M6.38086 15.8799L7.13086 16.6299L9.38086 14.3799" stroke="#00556e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M9 22H15C20 22 22 20 22 15V9C22 4 20 2 15 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22Z" stroke="#00556e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
          </div>
        </div>
      ),
    },
...
><svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M15.197 3.35462C16.8703 1.67483 19.4476 1.53865 20.9536 3.05046C22.4596 4.56228 22.3239 7.14956 20.6506 8.82935L18.2268 11.2626M10.0464 14C8.54044 12.4882 8.67609 9.90087 10.3494 8.22108L12.5 6.06212" stroke="#00556e" stroke-width="1.5" stroke-linecap="round"></path> <path d="M13.9536 10C15.4596 11.5118 15.3239 14.0991 13.6506 15.7789L11.2268 18.2121L8.80299 20.6454C7.12969 22.3252 4.55237 22.4613 3.0464 20.9495C1.54043 19.4377 1.67609 16.8504 3.34939 15.1706L5.77323 12.7373" stroke="#00556e" stroke-width="1.5" stroke-linecap="round"></path> </g></svg></div>
    ...
    
    {
      ...
      {
      title: (<Typography content="Tên công việc" modifiers={["14x20", "500", "center", "main"]} styles={{textAlign:"left", marginLeft:"12px"}}/>),
      dataIndex: "task_name",
      align: "center",
      ...
      {
      title: (<Typography content="task_last_note" modifiers={["14x20", "500", "center", "main"]} styles={{textAlign:"left", marginLeft:"12px"}}/>),
      dataIndex: "Ghi chú công việc lần cuối",
      align: "center",
      ...
      render: (record: any, data: any) => (
        ...
        {
      title: (<Typography content="Mô tả công việc" modifiers={["14x20", "500", "center", "main"]} styles={{
        marginLeft:15
      }}/>),
...
/>
  ...
  }, [currentStepId, dataBeforeExam?.data, loadingBefore, loadingListTask,dataListTask,dataListGuid,dataLog,listNotesCustomer]);
  // 
    const tableTaskLeads = useMemo(() => {
    return (
      <>
       ...
       <PublicTable
            ...
            column={tableTaskColumns}
            listData={dataTaskOfDay}
            isHideRowSelect
            scroll={{ x: 'max-content', y: '70vh - 80px' }}
    ...
    }, [dataTaskOfDay,stateStyleOfTask]);
      const tableTaskLeadsMyTask = useMemo(() => {
    return (
      <>
  ...
  const statisticHeader = useMemo(
    () => (
      <PublicHeaderStatistic handleClick={(data: any) => { }} title="Danh sách khách hàng tiềm năng" isStatistic={false} valueRangeDate={{ from: new Date(), to: new Date(), }} >
        {
          stateBreakPoint > 924 ?
            <div style={{paddingTop:"5px",paddingBottom:"5px"}}>
{/*             
                 <Button
        style={{ width: "100%", padding: "10px 0px" , display:"flex", justifyContent:"center", alignItems:"center",gap:10}}
        modifiers={["orange"]}
        onClick={async () => {
          handlePostReallocate()
        }}
      >
       
                <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#00556e"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <circle opacity="0.5" cx="12" cy="12" r="10" stroke="#ffffff" stroke-width="1.5"></circle> <path d="M17 10L7 10L10.4375 7" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M7 14L17 14L13.5625 17" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                <Typography content="Phân bổ xoay vòng" modifiers={["400"]} styles={{marginLeft:5}} />
      ...
      >
                  ...
                  <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <circle opacity="0.5" cx="12" cy="12" r="10" stroke="#fffff" stroke-width="1.5"></circle> <path d="M17 10L7 10L10.4375 7" stroke="#fffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M7 14L17 14L13.5625 17" stroke="#fffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                  <Typography content="Phân bổ xoay vòng" modifiers={["400"]} />
  ...
  return (
                      ...
                      >
                        {
                          stateBreakPoint < 1440 ?
                            ...
                            <div style={{display:"flex", justifyContent:"center", alignItems:"center", gap:5}}>
                              <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12.75 9C12.75 8.58579 12.4142 8.25 12 8.25C11.5858 8.25 11.25 8.58579 11.25 9L11.25 11.25H9C8.58579 11.25 8.25 11.5858 8.25 12C8.25 12.4142 8.58579 12.75 9 12.75H11.25V15C11.25 15.4142 11.5858 15.75 12 15.75C12.4142 15.75 12.75 15.4142 12.75 15L12.75 12.75H15C15.4142 12.75 15.75 12.4142 15.75 12C15.75 11.5858 15.4142 11.25 15 11.25H12.75V9Z" fill="#ffffff"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M12 1.25C6.06294 1.25 1.25 6.06294 1.25 12C1.25 17.9371 6.06294 22.75 12 22.75C17.9371 22.75 22.75 17.9371 22.75 12C22.75 6.06294 17.9371 1.25 12 1.25ZM2.75 12C2.75 6.89137 6.89137 2.75 12 2.75C17.1086 2.75 21.25 6.89137 21.25 12C21.25 17.1086 17.1086 21.25 12 21.25C6.89137 21.25 2.75 17.1086 2.75 12Z" fill="#ffffff"></path> </g></svg>
                               <Typography
                              content="Thêm lead"
                              modifiers={["400"]}
                            />
    ...
    abandoned_days: filterData.abandoned_days,
        ...
        iconName="search"
                              /></div>
                              <div style={{marginBottom:2, marginLeft:20,minWidth:230}}>
                              <Dropdown4
                      dropdownOption={[
                    ...
                    />
                              ...
                              <Checkbox
                                                   label="DS khách đã bị bỏ quên 3 ngày!"
                                                   isChecked={filterData.is_abandoned}
                                                   onChange={(data) => {
                                                    setFilterData({ ...filterData, is_abandoned: data?.target?.checked });
                                                    dispatch(
                                                      getListToStoreBeforeExams({
                                                        ...bodyGetList,
                                                        is_abandoned: data?.target?.checked,
                                                      } as unknown as any))
      ...
      {/* {
       ...
       <CModal
        ...
        <div style={{ maxHeight: "70vh" }}>
          {
            stateStyleOfTask === "my_task" ? tableTaskLeadsMyTask : tableTaskLeads
          }
             
                </div>
            </CModal >
   <TaskSummaryPopup
  openListTask={openListTask}
  setOpenListTask={setOpenListTask}
  data={dataListTaskFlo?.data} // chính là object bạn gửi ở trên
        setDataTaskOfDay={setDataTaskOfDay}
        styleOfTask={setStateStyleOfTask}
/> 
