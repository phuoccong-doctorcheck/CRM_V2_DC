...
const ScreeningReminder: React.FC = () => {
 ...
 const [seenListReminder, setSeenListReminder] = useState({
  ...
  const handleChangePagination = (page: number, size: number) => {
   ...
   useEffect(() => {
      document.title = 'Chuyển đổi F2,F3,WOM | CRM'
    }, []);
  const columnTable = [
    {
   ...
   {
  title: (
    <Typography content="Nguồn" modifiers={['12x18', '500', 'center', 'uppercase']} />
  ),
    ...
    },
    {
      ...
      render: (record: any, data: any) => (
       <div className="ant-table-column_item" style={{ display:"flex", justifyContent:"center", alignItems:"center"}}>
          <CTooltip placements="top" title="Cập nhật nhắc tầm soát">    <Icon iconName="edit_info" onClick={() => {
            setSendNote({
    ...
    <div className='p-customer_f_type'>
      <PublicLayout>
        <PublicHeader
          ...
          tabLeft={
            <div>
              <div style={{color:"#00556e", fontWeight:"600", fontSize:"16px"}}><span style={{color:"red"}}>Lưu ý:</span>  + Nếu KH đến sau 1 tháng kể từ lần đầu tới thì khuyến nghị nhắn tin hoặc liên hệ KH để chuyển sang F2 & WOM</div>
              <div style={{marginLeft:"44px",color:"#00556e", fontWeight:"600", fontSize:"16px"}}>       + Nếu KH đến sau 5 tháng kể từ lần đầu tới thì khuyến nghị nhắn tin hoặc liên hệ KH để chuyển sang F3</div>
            </div>
          }
           listBtn={
...
/>
      ...
      >
        <>
            ...
            <div className="t-header_wrapper-merge_customer_wrapper">
              {/* {formMergeCustomer} */}
               <div className='p-customer_leads_form_sms'>
                ...
                <Dropdown label='Trạng thái' dropdownOption={dataStatus || []} variant='simple' isRequired error={sendNoteEror.status} handleSelect={(item) => {
          ...
          <TextArea error={sendNoteEror.note} label='Ghi chú' id={''} readOnly={false} isResize
            value={note}
         ...
         <Button  modifiers={['foreign']} onClick={handleExcuteSendNote}>
                <Typography>Cập nhật Note</Typography>
                    </Button>
