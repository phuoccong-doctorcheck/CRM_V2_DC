...
const InteractionHistoryRC: React.FC<InteractionHistoryProps> = ({ options, id, isPopup, get5Item, loadingNote, handleChangeType, typeNote ,chatBox}) => {
        ...
        {loading ? (
          ...
          <div className={mapModifiers('o-interact_history_content_item',)} key={index}>
            <div className="o-interact_history_content_item_person">
              <Typography content={note.employee_name} />
              <div className={mapModifiers('o-interact_history_content_item_person_bottom')}>
              
                <span>{moment(note.note_datetime).format('HH:mm - DD/MM/YYYY')}</span>
              </div>
            </div>
            <div className="o-interact_history_content_item_note">
              <RichTextEditor data={note.cs_notes} isDisabled typeText='notHeadernotBordernotBGBoxShadown' />
            </div>
          </div>
...
InteractionHistoryRC.defaultProps = {
  isPopup: false,
};

export default InteractionHistoryRC;
