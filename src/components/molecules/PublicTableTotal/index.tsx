...
const PublicTableTotal: React.FC<PublicTableProps> = ({
 ...
 function totalConut(listData: any) {
  let total = 0;

   listData.forEach((group: any, groupIndex: number) => {
      console.log(group)
        console.log(group?.service_group_item?.length)
      // Thêm logic để sử dụng index nếu cần
      total += group?.service_group_item?.length;  // Nếu item không có 'length', ta cộng thêm 0
  ...
  function totalUnitPrice(listData:any) {
   ...
   function totalCustomerRevenue(listData:any) {
    ...
    listData.forEach((group:any) => {
      group.service_group_item.forEach((item:any) => {
        total += ( item.customer_revenue * item.quantity - item.commission_value * item.quantity);
      });
    });
        ...
        summary={() => (
          <Table.Summary>
            ...
            <Table.Summary.Cell index={column.length}  className="ant-table-summary-cell">
              <Typography content="" modifiers={['13x18', '600', 'center', 'main']} styles={{paddingTop:"5px", paddingBottom:"5px"}}/>
           </Table.Summary.Cell>
            <Table.Summary.Cell index={column.length}  className="ant-table-summary-cell">
              <Typography content="Tổng tiền" modifiers={['13x18', '600', 'center', 'main']} styles={{paddingTop:"5px", paddingBottom:"5px"}}/>
           </Table.Summary.Cell>
            <Table.Summary.Cell index={1} colSpan={column.length - 6} className="ant-table-summary-cell">
             ...
             <Typography content={totalConut(listData)?.toLocaleString("vi-VN")} modifiers={['13x18', '600', 'center', 'cg-red']} styles={{textAlign:"center"}}/> 
            </Table.Summary.Cell>
               <Table.Summary.Cell index={column.length - 1} className="ant-table-summary-cell">
