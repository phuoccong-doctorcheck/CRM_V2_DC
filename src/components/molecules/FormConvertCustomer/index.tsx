...
const FormConvertCustomer: React.FC<FormAddCustomerProps> = ({
  handleClose,
  handleAddCustomer,
  valUpdate,
  isUpdate,
  ...
  console.log("city:___________",dataForm.city,listProvince)
      useEffect(() => {
    ...
    if (isBooking) {
      if (serviceSelected.length !== 0) {
        const request = {
          customer: {
            ...
            //   valueUpdateCustomer?.customer?.ward_id?.id,
            province_id:
      ...
      const request = {
        customer: {
          ...
          customer_type:"customer",
            customer_fullname: dataForm.name,
            customer_identity_card:"12324566785",
            customer_phone: dataForm.phoneNumber,
            customer_email: dataForm.email || "",
            customer_address:        dataForm.address,
            day_of_birth:parseInt(dataForm.lead_dob, 10)|| undefined,
 ...
 const { mutate: getProvincesNew } = useMutation(
  ...
  console.log("valueUpdateCustomer______",valueUpdateCustomer,valUpdate)
  useEffect(() => {
    setDataForm({
      ...dataForm,
  ...
  useEffect(() => {
    setDataForm({
      ...
      district: (listDistrict ?? []).find(
        (i) =>
          i.value ==
          (valueUpdateCustomer?.district_id ||
            valUpdate?.district_id)
      ) as any,
    });
  ...
  useEffect(() => {
    ...
    if (isUpdate) {
      if (valUpdate?.province_id) {
        getDistricts(valUpdate?.province_id);
      }
...
FormConvertCustomer.defaultProps = {
...
export default FormConvertCustomer;
