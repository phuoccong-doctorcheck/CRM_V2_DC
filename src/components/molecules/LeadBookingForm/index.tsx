...
export default function LeadBookingForm({ data ,lead_id}: Props) {
          ...
          onSuccess: (data) => {
            setLoading(false)
             dispatch(getListAnswerMaster({
                ...
                toast.success("Lưu giai đoạn đặt hẹn thành công")
          },
          onError: (error) => {
  ...
  return (
    <div style={containerStyle}>
      <div style={contentStyle}>
        ...
        <h2 style={sectionTitleStyle}>Giai đoạn đặt lịch:</h2>
  ...
  const selectedQ1 = formData["q1"];
  const selectedQ6 = formData["q6"] === "true";

  // Logic ẩn/hiện
 // Điều kiện ẩn câu hỏi
if (
  (code === "q2" && !["Khám tổng quát", "Nội soi dạ dày", "Nội soi đại tràng"].includes(selectedQ1)) ||
  (code === "q3" && !["Nội soi dạ dày", "Nội soi đại tràng"].includes(selectedQ1)) || // bỏ BHYT nếu khám tổng quát
  (code === "q4" && !["Khám tổng quát", "Nội soi dạ dày", "Nội soi đại tràng"].includes(selectedQ1)) ||
  (["q5", "q6"].includes(code) && selectedQ1 !== "Nội soi đại tràng") ||
  (["q7", "q8"].includes(code) && (!selectedQ6 || selectedQ1 !== "Nội soi đại tràng"))
) {
  return null;
}
          ...
          <div key={opt}>
            <label>
              <input
                ...
                name={code}
                checked={value === opt}
                onChange={() => handleInputChange(code, opt)}
              />{" "}
              {opt}
            </label>
          </div>
...
<span>          Lưu giai đoạn đặt hẹn</span></>
              }
