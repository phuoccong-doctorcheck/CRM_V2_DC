...
const FormAddCustomerNew: React.FC<FormAddCustomerProps> = ({
  handleClose,
  handleAddCustomer,
  valUpdate,
  isUpdate = false,
    ...
    if (isBooking) {
      if (serviceSelected.length !== 0) {
        const request = {
          ...
          master: {
            ...
            is_register_subclinical:
              dataForm.typeBooking?.is_register_subclinical,
          ...
          is_appointment: isBooking || false,
          allow_update_profile: dataForm.allow_update_profile,
          c_schedule_id: idC_Schedule,
          //   appointment: {
          ...
          ids: [
            ...
            : ["package"].includes(dataForm.typeBooking?.value)
              ? serviceIds
              : dataForm.typeBooking?.id,
          script_answer_json: converContent
            ? converContent
            : dataCustomerPortrait,
          tags: [],
      ...
      const request = {
        ...
        province_id:
              isAddressOld ? dataForm.city?.value.toString() : dataForm.cityNew?.value.toString(),
            district_id:  isAddressOld ? dataForm.district?.value.toString() : null,
            ward_id:isAddressOld ? dataForm.ward?.value.toString() : dataForm.wardNew?.value.toString(),
          // launch_source_group_id: dataForm?.originGroup?.value,
          // launch_source_id: dataForm?.origin?.value,
          // launch_source_type_id: dataForm?.originType?.value,
          conversation_type: "",
  ...
  const { mutate: getCUstomerWoM } = useMutation(
    ...
    (data: any) => getCustomerByKey(data),
    {
      ...
      onError: (err) => {
        console.error("Error🚀 line 348 -> FormAddCustomerNew:", err);
      },
    }
  );
        ...
        setInsuranceData({
          ...
          content: (
            <>
              ...
              >
                 <Typography content={`KẾT QUẢ: ${(data?.data.maKetQua === '000' || data?.data.maKetQua === '004') ? 'Được áp dụng tại DoctorCheck' : 'Không được áp dụng tại DoctorCheck'}`} 
                  modifiers={[
                    "600",
                    data?.status && (data?.data?.maKetQua === "000" || data?.data?.maKetQua === "004")
                      ? "green"
                      : "cg-red",
              ...
              <ul
                className={mapModifiers(
                  ...
                  data?.status && ( data?.data?.maKetQua == "000" || data?.data?.maKetQua === "000")
                    ? "active"
                    : "error"
                )}
                style={{ marginTop: 6 }}
        ...
        setDataForm({
          ...
          typeBooking: valUpdate?.is_has_booking
            ? (stateAppointmentTypes.find(
              (i) =>
                i.value ===
                (valUpdate?.master?.appointment_type === "endoscopics" || valUpdate?.master?.appointment_type === null
                  ? "services"
                  : valUpdate?.master?.appointment_type)
            ) as GroupRadioType)
            : ({
            ...
            master_id: valUpdate?.master?.master_id || "",
          serviceAllowTypeBooking1: undefined as unknown as DropdownData,
              ...
              {!csPortrait && (
                <div className="m-form_add_customer_wrap">
                  ...
                  {/* Đây là layout search khách hàng cũ giới thiệu */}
                   {![8].includes(Number(dataForm?.origin?.value)) && (<div
                                    ...
                                    >
                                      {
                                        ( Number(dataForm?.origin?.value) === 6 || Number(dataForm?.origin?.value) === 9 || Number(dataForm?.origin?.value) === 11 || Number(dataForm?.origin?.value) === 7  || Number(dataForm?.origin?.value) === 10  ) && (
                                           <Checkbox
                  ...
                  {!isShowMore ? (
                    ...
                    <div>
                     ...
                     {/* <div
                      ...
                      {/* <div className="m-form_add_customer_row grid_2_1_1_1_1 m-form_add_customer_address" style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr"}}>
                        <AddressDropdown
                          AddressOption={listProvince || []}
                          label="Thành phố:"
                          handleSelect={(item, option: any) => {
...
FormAddCustomerNew.defaultProps = {
...
export default FormAddCustomerNew;
