...
const FormUpdateLead: React.FC<FormAddCustomerProps> = ({
  handleClose,
  handleAddCustomer,
  valUpdate,
  isUpdate,
  ...
  const storageADSAccounts = localStorage.getItem("ads_accounts");
          ...
          for (const adset of camp.adsets) {
            for (const ad of adset.ads) {
              if (ad.ad_id === dataForm.ad_id) {
                 console.log(acc.ads_account_id,camp.campaign_id,adset.adset_id,ad.ad_id)
  ...
  const [stateEndoscopics, setstateEndoscopics] = useState<DropdownData[]>(
    ...
    const [stateADSAccounts, setStateADSAccounts] = useState<DropdownData[]>(
    storageADSAccounts ? JSON.parse(storageADSAccounts) : []
  );
  console.log("stateADSAccounts", stateADSAccounts);
  ...
  // Side effect when resize
         useEffect(() => {
            if (valUpdate?.lead_province_id !== "") {
              getDistrictsNew(valUpdate?.lead_province_id);
            }
          }, [valUpdate?.lead_province_id]);
  useEffect(() => {
  ...
  const onSubmit = () => {
    ...
    const request = {
      ...
      step_id: dataForm?.step_id,
      ad_id: selectedAd?.value,
      is_converted: dataForm?.is_converted,
      lead_pancake_link: dataForm?.lead_pancake_link,
      lead_gender_id: dataForm.gender?.value || null,
          lead_id: dataForm.id,
          lead_name: dataForm.name,
          lead_phone: dataForm.phoneNumber,
          lead_email: dataForm.email || "",
          lead_address:           dataForm.address,
            ...
            lead_ward_id:isAddressOld ? dataForm.ward?.value.toString() : dataForm.districtNew?.value.toString(),
              source_group_id: dataForm?.originGroup?.id,
      ...
      source_type_id: dataForm?.originType?.value,
          ...
          bg_color: "#ffcc00",
          attribute_jsons: "{\"interested_service\": \"Tẩy trắng răng\", \"budget\": \"2tr\"}",
  ...
  useEffect(() => {
    ...
    getProvinces("VN");  getProvincesNew("VN");
  }, []);
  ...
  useEffect(() => {
    setDataForm({
      ...
      district: (listDistrict ?? []).find(
        (i) =>
          i.value ==
          (valueUpdateCustomer?.lead_ward_id ||
            valUpdate?.lead_ward_id)
      ) as any,
    });
  ...
  useEffect(() => { 
      ...
      const delay = setTimeout(() => {
        ...
        console.log(valUpdate.lead_id)
        setDataForm({
          ...dataForm,
          visit_id: valUpdate.visit_id,
          lead_pancake_id: valUpdate?.lead_pancake_id,
          ...
          lead_dob: valUpdate?.lead_dob?.toString(),
    lead_mob: valUpdate?.lead_mob?.toString(),
    lead_yob: valUpdate?.lead_yob?.toString(),
    ...
    is_converted: valUpdate?.is_converted,
          ...
          origin:
            (stateLaunchSource.find(
              (i) => i.id == valUpdate?.customer?.launch_source?.id
            ) as any),
          originGroup:
            (stateLaunchSourceGroups.find(
              (i) => i.id == valUpdate?.customer?.source_group_id
            ) as any) ,
  ...
  console.log((isUpdate && !_.isEmpty(dataForm?.id.toString())),!_.isEmpty(dataForm?.id.toString()))
  return (
    <div className="m-form_add_customer_wrapper">
      ...
      >
        ...
        >
          {(isUpdate && !_.isEmpty(dataForm?.id.toString())) || !isUpdate ? (
            <form>
              {!csPortrait && (
                <div className="m-form_add_customer_wrap">
                  {
                 (   dataForm?.lead_pancake_id === "" ||dataForm?.lead_pancake_id === null ) ? <></> :    <div
                    style={{
                  ...
                  >
                    <Typography
                      content="Luu ý: Thông tin bạn điều chỉnh tại đây sẽ không được đồng bộ ngược qua Pancake"
                      modifiers={["13x18", "cg-red", "500"]}
                      styles={{
                        fontSize: "14px",
              ...
              <div className="m-form_add_customer_button">
                ...
                >
                  ...
                  <Typography
                    type="span"
                    modifiers={["400", "16x24"]}
                    content={isUpdate ? "Cập nhật" : "Cập nhật"}
                    styles={{ marginBottom: "2px" }}
                  />
                </div>
              </div>
