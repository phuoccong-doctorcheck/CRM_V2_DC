...
const InputHo: React.FC<InputProps> = ({ type, error, label, id, variant, iconName, iconSize, handleClickIcon, disabled, isPassword, readOnly, isRequired, ValDefault, isFlex, isCategorySearch, isTodo, value, suffix, className,
  ...
  return (
    <div className={mapModifiers('a-input', className, variant, type, disabled && 'disabled', iconName && 'hasIcon', error && 'isError', readOnly && 'readOnly', isFlex && 'flexbox', isCategorySearch && 'search_categories', isLoading && 'loading', isSearch && 'search')} >
      {label && (
        <label style={{marginBottom:"4px"}} className={mapModifiers("a-input_label", isPhoneNumber && Number(value?.length) >= 10 && Number(value?.length) <= 12 && 'flex')} htmlFor={id}>
      ...
      <div className={mapModifiers('a-input_wrap', !!isCategorySearch && 'categories', type === 'phone_number' && 'phone_number', variant === 'todo_list' && 'todo_list')} style={{display:"flex",flexDirection:"row"}}>
        <div>
           {label}
